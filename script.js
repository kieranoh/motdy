...
function getThisWeekMonday8() {
  ...
  mon.setHours(0, 0, 0, 0);  // 매주 예약 조건 시간
  return mon;
}
...
document.getElementById('formBooking').onsubmit = async e => {
  ...
  // ---- 여기부터: 레이스 컨디션 방지용 "늦게 들어온 예약 자동 취소" 로직 ----
  const newRef = await rdb.ref('bookings').push(data);
  const myKey = newRef.key;

  if (!isAdmin) {
    // 기존 로직은 그대로 두고, 추가로 한 번 더 서버 상태 기준으로 겹침 검사
    const toMins2 = t => t.split(':').reduce((h, m) => h * 60 + parseInt(m), 0);
    const sMin2 = toMins2(start);
    const eMin2 = toMins2(end);

    const snapAfter = await rdb.ref('bookings')
      .orderByChild('date')
      .equalTo(date)
      .once('value');

    const overlapping = [];
    snapAfter.forEach(ch => {
      const b2 = ch.val();
      const bs = toMins2(b2.start);
      const be = toMins2(b2.end);
      if (sMin2 < be && eMin2 > bs) {
        overlapping.push({ key: ch.key, data: b2 });
      }
    });

    if (overlapping.length > 1) {
      overlapping.sort((a, b) => (a.key < b.key ? -1 : 1));
      const winner = overlapping[0].key;

      // 내가 가장 먼저 생성된 예약이 아니면 → 내 예약 자동 취소
      if (myKey !== winner) {
        await newRef.remove();
        alert('이미 같은 시간대에 다른 예약이 먼저 완료되어, 내 예약은 취소되었습니다.');
        btnCancel.onclick();
        renderSchedule();
        return;
      }
